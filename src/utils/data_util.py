...
sys.path.append("/users/grad/lisusi/tr.lisusi/gnn_init_rec-master/")
from src.datasets.data_load import (
    load_item_fea_dic,
    load_split_dataset,
    load_user_item_feature,
)
from src.utils.aliasTable import AliasTable
from src.utils.common_util import ensureDir, get_random_rep
from src.utils.constants import DEFAULT_ITEM_COL, DEFAULT_RATING_COL, DEFAULT_USER_COL
from src.utils.triple_sampler import Sampler
...
class Dataset(object):
    ...
    def load_user_item_fea(self):
        ...
        user_feat_dic, item_feat_dic = load_user_item_feature(self.config)

        user_feat_li = []
        item_feat_li = []
        for user in range(self.n_users):
            user_feat_li.append(user_feat_dic[self.id2user[user]])
        for item in range(self.n_items):
            item_feat_li.append(item_feat_dic[self.id2item[item]])
    ...
    def make_multi_graph(self, source):
        ...
        item_edge_list = np.array([item_edge_s, item_edge_e])
        item_edge_type = np.array(item_edge_type)

        print(
            f"load user_edge_list:{user_edge_list.shape},user_edge_type:{user_edge_type.shape}"
        )
        print(
            f"load item_edge_list:{item_edge_list.shape},item_edge_type:{item_edge_type.shape}"
        )
        return (
            user_edge_list,
            user_edge_type,
